...
################################################################
#################### EARLY-GRADE MATH TOOLS ####################
fun num-negate(x :: Number): -1 * x end
negate = num-negate
abs = num-abs
...
fun time-text(str):
  text(str, 14, color(41, 128, 38, 255))
end

fun height-text(str):
  text(str, 14, color(38, 38, 128, 255))
end
...
axis = put-image(flip-horizontal(tri), 
  STRIP-WIDTH + 38, STRIP-HEIGHT / 2, 
  put-image(tri, 10, STRIP-HEIGHT / 2, inequalities-bg))
...
fun dot-builder(draw-point, project, points, base):
  points.foldl(
    lam(p, img):
      put-image(draw-point(p), project(p) + 24, STRIP-HEIGHT / 2, img)
    end,
    base)
end
...
fun draw-inequality(points, f, msg-img) block:
  ...
  axis-strip = put-image(draw-shade(f, rProject, c1),
  ...
  # add the points
  dots-strip = dot-builder(lam(p): put-image(
...
fun draw-being(b :: Being, background):
  ...
  put-image(b.costume, b.posn.x, b.posn.y, dbg-bkgnd)
end
...
fun animate-proc(title, title-color, background, 
  ...
  player = being(posn(GAME-WIDTH / 2, GAME-HEIGHT / 2), center-pinhole(playerImg))
  ...
  init-world = {
    dangers: for map(img from flatten([list: dangerImgs])):
        reset(being(posn(0, 0), center-pinhole(img)), update-danger)
      end, 
    shots: empty, 
    targets: for map(img from flatten([list: targetImgs])):
        reset(being(posn(0, 0), center-pinhole(img)), update-target)
      end, 
    player: player, 
    bg: background, 
    score: 100, 
  ...
  key-press = lam(w, key):
    ask:
      ...
      | key == " " then:
        w.{shots: link(being(posn(_player-x_, _player-y_), projectileImgs), w.shots)}
